...
module.exports = function (plugin, server) {
  ...
  plugin.status.yellow('Waiting for Engine');

  function waitForPong() {
    return client.ping().catch(function (err) {
      if (!(err instanceof NoConnections)) throw err;

      plugin.status.red(format('Unable to connect to Engine at %s.', config.get('elasticsearch.url')));
  ...
  function waitForShards() {
    ...
    .then(function (health) {
      if (health === NO_INDEX) {
        plugin.status.yellow('No existing Analytics index found');
        return createKibanaIndex(server);
      }

      if (health === INITIALIZING) {
        plugin.status.red('Engine is still initializing the Analytics index.');
        return Promise.delay(REQUEST_DELAY).then(waitForShards);
      }

      // otherwise we are g2g
      plugin.status.green('Analytics index ready');
    });
  }
